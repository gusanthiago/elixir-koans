defmodule Structs do
  ...
  koan "Use the put_in macro to replace a nested value" do
    airline = %Airline{}
    new_airline = %Airline{plane: %Plane{passengers: 0, maker: :airbus}, name: "Southwest"}

    assert put_in(airline.plane.maker, :airbus) == new_airline
  end

  koan "Use the update_in macro to modify a nested value" do
    airline = %Airline{plane: %Plane{passengers: 200}}
    new_airline = %Airline{plane: %Plane{passengers: 202, maker: :boeing}, name: "Southwest"}
    assert update_in(airline.plane.passengers, fn x -> x + 2 end) == new_airline
  end
